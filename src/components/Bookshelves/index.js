...
class Bookshelves extends Component {
  ...
  renderBooksList = () => {
    ...
    if (booksList.length < 2) {
      // Render a message indicating that there are not enough books
      return <p>There are not enough books to display.</p>
    }
    return (
      <BookHubThemeContext.Consumer>
        {value => {
              ...
              <ul className="books-list-container">
                ...
                {!isEmptyBooksList &&
                  booksList.map(eachBook => {
                    ...
                    return (
                      <Link to={`/books/${id}`} className="text-links" key={id}>
